...
export const useChannelsStore = create<ChannelsState>((set, get) => ({
  ...
  async fetchMyChannels() {
    ...
    if (!uid) { set({ channels: [] }); return; }

    const { data: mems, error: err1 } = await supabase
      .from("channel_members")
      .select("channel_id")
      .eq("user_id", uid);

    if (err1 || !mems?.length) { set({ channels: [] }); return; }
    ...
    if (err2) { console.error(err2); set({ channels: [] }); return; }
    set({ channels: chs ?? [] });
  },
  ...
  async createChannel(name: string) {
    ...
    const ins = await supabase.from("channel_members").insert({
      channel_id: data.id, user_id: uid, role: "owner"
    });
